	...
	: IRecipeService
{
	...
	public async Task<int?> GetCount(Guid userId, CancellationToken ct)
	{
	...
	{
		var currentUser = await userService.GetCurrent(ct);
		var favoritedRecipesData =
			await api.Api.Recipe.Favorited.GetAsync(config => config.QueryParameters.UserId = currentUser.Id,
				cancellationToken: ct);
