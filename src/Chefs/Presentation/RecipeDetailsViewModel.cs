...
public partial class RecipeDetailsViewModel // DR_REV: Use Model suffix instead of ViewModel
{
    ...
    public async ValueTask LiveCooking(IImmutableList<Step> steps, CancellationToken ct) =>
        await _navigator.NavigateViewModelAsync<LiveCookingViewModel>(this, data: new LiveCookingParameter((await Recipe)!, steps));

	// DR_REV: Use implicit parameters
	public async ValueTask IngredientsNavigation(IImmutableList<Ingredient> ingredients, CancellationToken ct) =>
        await _navigator.NavigateViewModelAsync<IngredientsViewModel>(this, data: new IngredientsParameter((await Recipe)!, ingredients));

	// DR_REV: Use implicit parameters
	public async ValueTask Review(IImmutableList<Review> reviews, CancellationToken ct)
    {
        _ = await _navigator.GetDataAsync<ReviewsViewModel>(this, data: new ReviewParameter((await Recipe)?.Id ?? Guid.Empty, reviews));
